...
def boar_brawl(player_score, opponent_score):
    ...
    tens = opponent_score // 10 % 10  # Don't assume that scores are below 100
    ones = player_score // 1 % 10
    return (3 * abs(tens - ones)) or 1
    # END PROBLEM 2
...
def num_factors(n):
    ...
    # BEGIN PROBLEM 4
    if n <= 0:
        return 0  # Factors are typically defined for positive integers
    ...
    for i in range(1, int(n**0.5) + 1):
        if n % i == 0:
            ...
            # If i * i == n, it means i is the square root, so n/i is also i.
            # In this case, we only count it once to avoid double-counting.
            if i * i == n:
                count += 1
            else:
...
def sus_points(score):
    ...
    # BEGIN PROBLEM 4
    is_sus_number = num_factors(score) in (3, 4)
    if is_sus_number:
        next_prime = score + 1
...
def sus_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    """
    # BEGIN PROBLEM 4
    take_turn_score = take_turn(num_rolls, player_score, opponent_score, dice)
    new_score = player_score + take_turn_score
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for score, opponent_score in all_combinations:
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    for num_rolls in range(1, 11):
        averaged_roll = make_averaged(roll_dice, times_called)(num_rolls, dice)
        if averaged_roll > max_avg:
            max_avg = averaged_roll
...
def final_strategy(score, opponent_score):
    ...
    This strategy combines the boar_strategy and sus_strategy with adjusted thresholds.
    It opts to roll 0 dice if the potential score increase from Boar Brawl
    and Sus Fuss is significant enough (threshold of 10 points).
    Otherwise, it defaults to rolling 6 dice.
    """
    # BEGIN PROBLEM 12

    potential_score = score + boar_brawl(score, opponent_score)
    if sus_points(potential_score) - score >= 10:
        return 0
    else:
        return 6
    # END PROBLEM 12
